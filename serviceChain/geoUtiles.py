...
class ImageManager:
    ...
    # 保存图片（公共方法）
    def saveImg(self, strSavePath=None, dictImages=None, strOutFormat='.jpg', formEE=False):
        # 如果未指定保存路径，使用桌面的result文件夹
        if strSavePath is None:
            import os.path
            ...
            # 在桌面创建result文件夹
            strSavePath = os.path.join(desktop_path, "result")
            print(f"未指定保存路径，将使用默认路径: {strSavePath}")
        
        if not os.path.exists(strSavePath):
            os.makedirs(strSavePath)
            print(f"创建保存目录: {strSavePath}")

        # 如果未指定要保存的字典，根据最近操作自动选择
        if dictImages is None:
            # 按照优先级选择要保存的数据源
            if self.dictGeoReferencedImages:
                dictImages = self.dictGeoReferencedImages
                print("检测到地理参考数据，自动选择保存地理参考影像")
            elif self.dictAppendedImages:
                dictImages = self.dictAppendedImages
                print("检测到追加的数据，自动选择保存追加影像")
            elif self.dictConvertedImages:
                dictImages = self.dictConvertedImages
                print("检测到拉伸处理数据，自动选择保存拉伸影像")
            elif self.dictStitchedImages:
                dictImages = self.dictStitchedImages
                print("检测到拼接数据，自动选择保存拼接影像")
            elif self.dictCroppedImages:
                dictImages = self.dictCroppedImages
                print("检测到裁剪数据，自动选择保存裁剪影像")
            else:
                dictImages = self.dictImages
                print("使用原始影像数据")

        # 确保文件扩展名以点开头
        if not strOutFormat.startswith('.'):
            strOutFormat = '.' + strOutFormat
        
        # 检查扩展名是否为有效格式
        valid_extensions = ['.jpg', '.jpeg', '.png', '.bmp', '.tif', '.tiff']
        if strOutFormat.lower() not in valid_extensions:
            print(f"警告: 不支持的文件格式 {strOutFormat}，使用默认格式 .jpg")
            strOutFormat = '.jpg'
        ...
        for strKey, objValue in dictImages.items():
            ...
            print(f"保存图像至: {savePath}")
        
        print(f"共保存了 {len(dictImages)} 张图像到 {strSavePath} 目录")
